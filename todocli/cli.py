...
def init(
    ...
    if app_init_error:
        typer.secho(
            f'Creating configuration file failed with "{ERRORS[app_init_error]}"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
...
def list_all() -> None:
    ...
    for todo_number, todo in enumerate(todo_list, 1):
        description, priority, done = todo.values()
        typer.secho(
            f"{todo_number}{(len(columns[0]) - len(str(todo_number))) * ' '}"
